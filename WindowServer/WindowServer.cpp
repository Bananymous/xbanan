...
void WindowServer::invalidate_impl(Rectangle area)
{
			...
			if (!should_alpha_blend)
			{
				for (int32_t y = area.y; y < area.y + area.height; y++)
					for (int32_t x = area.x; x < area.x + area.width; x++)
						m_framebuffer.pixels[y * m_framebuffer.width + x] = m_focused_window->framebuffer()[y * m_focused_window->client_width() + x];
			}
			else
			{
				for (int32_t y = area.y; y < area.y + area.height; y++)
				{
					for (int32_t x = area.x; x < area.x + area.width; x++)
					{
						...
						m_framebuffer.pixels[y * m_framebuffer.width + x] = alpha_blend(src_pixel, bg_pixel);
					}
				}
			}
			mark_pending_sync(area);
		...
		else
		{
			...
			for (int32_t dst_y = dst_area.y; dst_y < dst_area.y + dst_area.height; dst_y++)
			{
				for (int32_t dst_x = dst_area.x; dst_x < dst_area.x + dst_area.width; dst_x++)
				{
					...
					uint32_t& dst_pixel = m_framebuffer.pixels[dst_y * m_framebuffer.width + dst_x];
		...
		if (!m_is_mouse_relative)
		{
			...
			for (int32_t rel_y = 0; rel_y < cursor_area.height; rel_y++)
			{
				for (int32_t rel_x = 0; rel_x < cursor_area.width; rel_x++)
				{
					...
					m_framebuffer.pixels[dst_y * m_framebuffer.width + dst_x] = pixel.value();
				}
			}
	...
	for (int32_t y = area.y; y < area.y + area.height; y++)
		for (int32_t x = area.x; x < area.x + area.width; x++)
			m_framebuffer.pixels[y * m_framebuffer.width + x] = m_background_image->get_color(x, y).as_argb();
		...
		if (auto title_overlap = window.title_bar_area().get_overlap(area); title_overlap.has_value())
		{
			for (int32_t y_off = 0; y_off < title_overlap->height; y_off++)
			{
				for (int32_t x_off = 0; x_off < title_overlap->width; x_off++)
				{
					...
					m_framebuffer.pixels[abs_y * m_framebuffer.width + abs_x] = color;
				}
			}
		}
		...
		if (auto client_overlap = window.client_area().get_overlap(area); client_overlap.has_value())
		{
			for (const auto& fast_area : fast_areas)
			{
				...
				for (int32_t y_off = 0; y_off < fast_overlap->height; y_off++)
				{
					...
					auto* frameb_row = &m_framebuffer.pixels[  abs_row_y * m_framebuffer.width   + abs_row_x];
			...
			for (const auto& corner_area : corner_areas)
			{
				...
				for (int32_t y_off = 0; y_off < corner_overlap->height; y_off++)
				{
					for (int32_t x_off = 0; x_off < corner_overlap->width; x_off++)
					{
						...
						const uint32_t color_b = m_framebuffer.pixels[abs_y * m_framebuffer.width + abs_x];

						const bool should_alpha_blend = window.get_attributes().alpha_channel;
						m_framebuffer.pixels[abs_y * m_framebuffer.width + abs_x] = should_alpha_blend
							? alpha_blend(color_a, color_b)
							: color_a;
					}
				}
	...
	if (m_state == State::Resizing)
	{
		if (const auto overlap = resize_area(m_cursor).get_overlap(area); overlap.has_value())
		{
			for (int32_t y_off = 0; y_off < overlap->height; y_off++)
			{
				...
				{
					auto& pixel = m_framebuffer.pixels[(overlap->y + y_off) * m_framebuffer.width + (overlap->x + x_off)];
...
{
	m_dirty_rect = m_dirty_rect.get_bounding_box(to_sync).get_overlap(m_framebuffer.area()).value_or({});
}
...
void WindowServer::sync()
{
	...
	if (m_dirty_rect.width == 0 || m_dirty_rect.height == 0)
		return;

	invalidate_impl(m_dirty_rect);

	const SDL_Rect rect {
		.x = m_dirty_rect.x,
		.y = m_dirty_rect.y,
		.w = m_dirty_rect.width,
		.h = m_dirty_rect.height,
	};

	m_dirty_rect = {};

	SDL_UpdateTexture(g_texture, &rect, m_framebuffer.pixels, m_framebuffer.width * 4);
